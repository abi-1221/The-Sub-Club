	...
	return (
		<footer className="border-t border-white/10 bg-black/40 backdrop-blur-md">
			<div className="mx-auto max-w-7xl px-4 lg:px-8">
					...
					{footerLinks.menu?.items?.map((item) => {
						return (
							<div key={item.id}>
								...
								<ul className="mt-4 space-y-4 [&>li]:text-neutral-400">
									{item.children?.map((child) => {
										if (child.category) {
											return (
												<li key={child.id} className="text-sm hover:text-white transition-colors">
				...
				<div className="flex flex-col justify-between border-t border-white/10 py-10 sm:flex-row">
					<p className="text-sm text-neutral-500">Copyright &copy; {currentYear} THE SUB CLUB. All rights reserved.</p>
					<p className="flex gap-1 text-sm text-neutral-500">
						...
						<Link href={"https://github.com/saleor/saleor"} target={"_blank"} className={"opacity-30 hover:opacity-100 transition-opacity"}>
							<Image alt="Saleor github repository" height={20} width={20} src={"/github-mark.svg"} className="invert" />
						</Link>
					</p>
				</div>
			</div>
